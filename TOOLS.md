...
- **QQID:** `EB55A88C3F8A812D7803064F48AB518F`
- **配置字段:** `commands.ownerAllowFrom: ["qqbot:EB55A88C3F8A812D7803064F48AB518F"]`
- **作用:** owner-only 命令权限（/diagnostics、exec approvals 等）
...
- **模型:** xiaomi/mimo-v2.5 (params: topP=0.2, temperature=0.2, frequencyPenalty=0.5)
...
### Memory Search — text-embedding-v4

- **Provider:** `openai-compatible`
- **Model:** `text-embedding-v4`（阿里云 DashScope，1024 维）
- **Endpoint:** `https://ws-ojg7j279jj38u49g.cn-beijing.maas.aliyuncs.com/compatible-mode/v1`
- **API Key:** 阿里云 DashScope API Key（存在 `openclaw.json` 中）
- **配置位置:** `agents.defaults.memorySearch`
- **索引重建:** `openclaw memory index --force --agent robot01`
- **状态检查:** `openclaw memory status --agent robot01`（注意：4 核小机上可能超时）
- **混合搜索:** 默认开启（BM25 + vector），vectorWeight=0.7, textWeight=0.3

### Dual Agent Flow 模型配置

| 角色 | 模型 | API | 端点 |
|------|------|-----|------|
| WorkAgent | mimo-v2.5 | 默认 | 默认 |
| MiddleAgent | mimo-v2.5-pro | 默认 | 默认 |
| AcceptAgent | deepseek-v4-flash | 默认 | 默认 |
